...
# the CI workspace should build against. A cross-repo change is developed on a branch
...
# predastore=<ref> and northstar=<ref> to $GITHUB_OUTPUT for the checkout steps
# to consume.
set -euo pipefail
...
for repo in viperblock predastore northstar; do
