...
| `spx admin operator-principal create` | — | Creates the `operator` IAM user in the super-admin account with an inline policy allowing `spinifex:CreateAccount`, `spinifex:DeleteAccount`, `spinifex:DescribeAccountDeletion` and `spinifex:ListAccounts` — each named rather than a wildcard, so a later addition to the admin surface is not authorised by an existing key. Revokes any access key left from a previous run and mints one new key. The secret is printed once; store it as an AWS profile. This is the credential the `--remote` admin commands sign with, and it is deliberately separate from the signup Worker's. |
...
Each method is a separate grant. The signup credential holds exactly `spinifex:CreateAccount` and cannot reach deletion with it; the other three methods need `spinifex:DeleteAccount`, `spinifex:DescribeAccountDeletion` and `spinifex:ListAccounts`, which is what `spx admin operator-principal create` mints. Keeping the two credentials separate is the point: the signup Worker's key is exposed to a public form, and revoking either costs nothing else.

Both surfaces are unreachable until an operator runs `spx admin signup-principal create` or `spx admin operator-principal create`; there is no config toggle. Revocation is `aws iam delete-access-key`, which is immediate and cluster-wide. `[signup] max_accounts` in `awsgw.toml` caps how many accounts may exist; it defaults to 128 when the section is absent, and 0 means uncapped.
