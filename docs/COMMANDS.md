...
| `spx admin images promote` | `--image-id` (required), `--yes` | Cluster must be running; predastore reachable. Target must be an account-owned AMI (`ImageOwnerAlias` is a 12-digit account ID). | Reads `ami-<id>/config.json`, validates the AMI is account-owned, then rewrites `ImageOwnerAlias` to `"system"` in-place. No block data is copied. The change takes effect immediately — the AMI becomes visible to all accounts via `DescribeImages`. Prompts for confirmation (skipped with `--yes`). Already-system AMIs are refused. | 1. Happy path: account-owned AMI → promoted, visible to all accounts<br>2. Already system-owned → refused<br>3. Missing/corrupt config → `InvalidAMIID.NotFound`<br>4. Invalid AMI ID prefix → `InvalidAMIID.Malformed` | **DONE** |
