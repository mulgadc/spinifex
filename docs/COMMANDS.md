...
- **TLS:** **required by default on both engines** — `rds.force_ssl` on `postgres18` and `require_secure_transport` on `mariadb11.8`, both boolean, modifiable and dynamic, both defaulting to `1`. Setting either to `0` in a parameter group restores plaintext without a reboot. Defaulting on is a **deliberate divergence** for MariaDB, which AWS leaves off. An instance created before this landed begins refusing plaintext at its next reboot or parameter apply, and a client that explicitly disables TLS has to be changed.
