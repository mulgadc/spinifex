...
REST-JSON, dispatched by HTTP method + path rather than by an action parameter,
so the routing table in `gateway/eks.go` is the authoritative list. Clusters run
K3s on control-plane VMs; nodegroups are EC2 instances joined to them.

| Command | Status |
|---------|--------|
| `create-cluster`, `describe-cluster`, `list-clusters`, `delete-cluster` | **DONE** |
| `create-nodegroup`, `describe-nodegroup`, `list-nodegroups`, `delete-nodegroup`, `update-nodegroup-config`, `update-nodegroup-version` | **DONE** |
| `create-addon`, `describe-addon`, `describe-addon-versions`, `list-addons`, `update-addon`, `delete-addon` | **DONE** |
| `create-access-entry`, `describe-access-entry`, `list-access-entries`, `update-access-entry`, `delete-access-entry` | **DONE** |
| `associate-access-policy`, `disassociate-access-policy`, `list-associated-access-policies`, `list-access-policies` | **DONE** |
| `tag-resource`, `untag-resource`, `list-tags-for-resource` | **DONE for cluster ARNs**; other EKS resource ARNs return `NotImplemented` |
| `update-cluster-config`, `update-cluster-version` | **NOT STARTED** (`NotImplemented`) |
| Identity provider configs: `associate-identity-provider-config`, `describe-identity-provider-config`, `list-identity-provider-configs`, `disassociate-identity-provider-config` | **NOT STARTED** (`NotImplemented`) |

Four routes on this surface are **internal, not AWS actions**, and are not
callable with an ordinary tenant credential: `PublishInternal` (the control-plane
VM broker, relaying bootstrap/state POSTs onto `eks.bus.*`/`eks.state.*`),
`WebhookTokenReview` (the `eks-token-webhook` posting bearer tokens for host-side
resolution), `ListInternalAddons` (the on-VM addon-sync agent fetching its staged
manifests) and `GetRecoveryDirective` (the on-VM k3s-recovery agent reading its
per-member directive at boot). They name the target account in the path rather
than taking it from the credential, so each requires a control-plane instance-role
session bound to the named cluster — a broad `eks:*` grant does not reach them.

Disaster recovery for a lost single control plane is an operator command, not an
API call: see `spx admin eks restore-snapshot` above.

---

## Resource Groups Tagging API (`tagging`)

JSON-1.1, dispatched from the action suffix of
`X-Amz-Target: ResourceGroupsTaggingAPI_20170126.<Action>`.

| Command | Status |
|---------|--------|
| `get-resources` | **DONE** — authorizes against `"*"`, which is correct here rather than a fail-open: the Resource Groups Tagging API defines no resource types of its own. A two-way completeness test pins that, so a later action cannot be added to this surface with a silent account-wide grant |
| `get-tag-keys`, `get-tag-values`, `tag-resources`, `untag-resources` | **NOT STARTED** (`InvalidAction`) |
