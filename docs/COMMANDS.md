...
Each method is a separate grant. The signup credential holds exactly `spinifex:CreateAccount` and cannot reach deletion with it; deletion needs `spinifex:DeleteAccount`, which the super-admin account's own `admin` user has through its AdministratorAccess policy.

`CreateAccount` is unreachable until an operator runs `spx admin signup-principal create`; there is no config toggle. Revocation is `aws iam delete-access-key`, which is immediate and cluster-wide. `[signup] max_accounts` in `awsgw.toml` caps how many accounts may exist; it defaults to 128 when the section is absent, and 0 means uncapped.
