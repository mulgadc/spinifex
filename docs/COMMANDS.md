...
Both are served from the same tables the control plane validates against, so an engine pin bump or a class-map change moves these answers with it. Neither reads per-account state and neither paginates: at most 2 engine versions and 12 orderable options, against an AWS minimum page size of 20.

- **Filters are applied, not parsed and dropped.** These two actions exist only to be filtered, so an ignored `engine` filter would return the other engine's rows with no way for the caller to tell. An unrecognised filter name is `InvalidParameterValue` against each action's own closed vocabulary, and the two vocabularies are disjoint where their typed parameters are — `status` is rejected on the orderable action and `db-instance-class` on the engine-versions one. A typed parameter and a filter naming the same field are both applied, so asking for `--engine postgres` and `Name=engine,Values=mariadb` together matches nothing.
- **A filter that matches nothing is an empty result.** `--engine-version 17` returns zero rows from both rather than the `InvalidParameterValue` `create-db-instance` gives it: a query for a row that does not exist is empty, while a request to build something that cannot exist is refused. On `describe-orderable-db-instance-options` an absent `--engine` is `MissingParameter` and an unknown one is `InvalidParameterValue`, because it is required.
- **`AvailabilityZones` is empty** on every orderable option. `AvailabilityZone` is a rejected create parameter, so naming a zone here would invite a request that fails. `AvailableProcessorFeatures` is empty for the same reason: `--processor-features` is accepted and ignored on create, so advertising one would promise a knob that does nothing.
- **A cluster that answers nothing is `ServerInternal`**, never the full class list — reporting classes that cannot launch is the failure the capability probe exists to prevent. A cluster that answers and runs none of the classes is an empty option list, which is the truthful answer for it.
- The probe describes what the platform offers, not what it has free right now. Exhausted capacity is a create-time failure, and filtering on it would make a Terraform data source return a different answer on every plan.

Three deliberate divergences from AWS:

| Divergence | Why |
|---|---|
| `--marker` is **rejected** on these two, while the other RDS describes parse and ignore it | Those may one day paginate, so a `Marker` they ignore is a forward-compatible no-op. These two never will, so a `Marker` here can only have been fabricated, and answering it as page one would report the whole catalog as a later page |
| `--default-only`, `--include-all`, `--list-supported-character-sets` and `--list-supported-timezones` are accepted as **provable identities**, a stronger claim than the "parsed, not applied" the other describes make | Every engine has exactly one version and that version is the default and is `available`, and both lists are empty by design, so none of the four can change the answer. A second version per engine, or a populated list, breaks the identity and fails the test that pins it |
| `--filters` on `describe-orderable-db-instance-options` is **applied**, and an unknown name rejected, where AWS documents the parameter as unsupported there | Stricter than AWS rather than looser: a filter AWS ignores returns rows the caller asked not to see. `describe-db-engine-versions` does support filters on AWS, and this matches its documented set |
