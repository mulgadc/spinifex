...
# The engine this preset bakes is stated in three places that have to agree: the
# stamp setup.sh writes, the manifest tag resolveEngineAMI selects on, and the
# name rds-agent's layout table is keyed by. A drift between them launches the
# wrong image or refuses the right one.
