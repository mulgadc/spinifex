...
func TestCreateAccessKey_SecretIsDecryptable(t *testing.T) {
	...
	decrypted, err := svc.key.Decrypt(ak.SecretAccessKey)
	require.NoError(t, err)
...
func TestSeedBootstrap(t *testing.T) {
	...
	encryptedSecret, err := svc.key.Encrypt("test-secret-key")
	require.NoError(t, err)
...
func TestSeedBootstrap_Idempotent(t *testing.T) {
	...
	encryptedSecret, _ := svc.key.Encrypt("test-secret")
	data := &BootstrapData{
		AccessKeyID:     "AKIAEXAMPLE123456789",
		EncryptedSecret: encryptedSecret,
		AccountID:       utils.GlobalAccountID,
...
func TestSeedBootstrap_WithAdmin(t *testing.T) {
	...
	systemSecret, err := svc.key.Encrypt("system-secret")
	require.NoError(t, err)
	adminSecret, err := svc.key.Encrypt("admin-secret")
	require.NoError(t, err)
...
func TestSeedBootstrap_AdminNil_BackwardCompat(t *testing.T) {
	...
	encryptedSecret, err := svc.key.Encrypt("test-secret")
	require.NoError(t, err)
